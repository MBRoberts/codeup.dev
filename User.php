...
class User extends Model
{
	...
	protected function insert()
	{
		...
		foreach ($this->attributes as $key => $attribute) {
			$query->bindValue(":{$key}", $attribute, PDO::PARAM_STR);
	    }
	...
	protected function update()
	{
		...
		foreach ($this->attributes as $key => $attribute) {
			$query->bindValue(":{$key}", $this->attribute["{$key}"], PDO::PARAM_STR);
	    }
	...
	public static function all()
	{
		...
		$query = self::$dbc->query('SELECT * FROM ' . self::$table);
	...
	public function delete()
	{
		...
		$query->bindValue(':id', $this->attribute['id'], PDO::PARAM_INT);
		$query->execute();
	}
}
$user = new User();
